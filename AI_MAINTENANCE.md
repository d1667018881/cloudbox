...
> 本文档最后更新：2026-09-25（新增 `docs/` 证据文件索引；§38 补作废标注；
> §9 两处旧结论加更正注。正文各节的 commit 标注保留原状）。
>
> 📎 **本文档引用的原始证据文件（逆向报告、设置项审计、断言审计、功能对照表）**
> **都在仓库 `docs/` 目录，入口见 `docs/README.md`。§39.5 提到的
> `audit_claims.md` / `audit_settings_defaults.md` 已入库，不再只存在于开发者沙箱。**
...
**A4. 定默认值 / 写「实测」「对齐原版」类断言之前，必须先找一手证据。**
（2026-09-24 新增，起因见 §39.1）

证据优先级：**原版明文源码 > 本人的一手实测 > 第三方说法 > 自己的推测**。

- 找不到一手证据时，取**最小干预**：不改名、不删除、不自动开启、不擅自扩大范围。
- 引用"实测"必须写明**被验证对象与样本数**。正确写法：「实测：**无扩展名**的文件被拒（n=1）」；
  错误写法：「实测 apk/exe 都被拒」——后者把 1 个窄样本推广成了整类。
- 声称"对齐原版"必须给出**原版文件:行**，不能凭印象；**手上有明文源码就去查，不要猜**。
- 涉及**用户可见行为**的默认值改动（尤其"自动开启某功能""对某类文件生效"），**先问用户**。

反面例子（同一天、同一根因，四连，全部真实发生）：
① 把「无扩展名被拒」推广成「apk/exe 都被拒」，据此把「上传改名」设成**默认开启**
  —— 而原版 `update_log.lua` 明文写着此功能**默认关闭**；
② 给**所有文件**插版本号（用户需求只说"给软件加版本号"）；
③ 照抄第三方项目 PR 的域名列表，混进 3 个假域（其中一个 DNS 根本不存在）；
④ 把「应用内语言」做成假功能（`strings.xml` 只有 1 条，选了 English 界面照样中文）。

> 共同的根因：**把推断写成断言，然后自己引用自己，推断就变成了"事实"**。
...
| **无证据断言**（2026-09-24 增） | 把 1 个样本推广成整类；照抄第三方说法当成事实；说"对齐原版"却凭印象；默认值自作主张 | **A4**：先找一手证据 + 写明样本数 + 给出原版文件:行；用户可见行为先问 |
...
仓库根另有：`AI_MAINTENANCE.md`（本文）、`README.md`、`CODE_REVIEW_V3.md`、
`CODE_REVIEW_V4_LOGIN.md`、**`docs/`（原始证据文件，入口 `docs/README.md`）**。
...
- **版本**：master 分支，commit `3bb917f`；2025 年仍活跃维护
- ⚠️ **更正（2026-09-24）**：本节原先还写了「PR#69 于 2025-10 修复 `lanzouo.com` 域名判定，
  说明接口持续有效」，**举的这个例子本身是错的** —— `lanzouo.com` 实测 443 直接
  `ConnectionRefused`、首页无蓝奏云内容，是**假域**（详见 §39.1 与本轮 domain 清理）。
  该 PR 的**其它内容**（task 编号等）不受影响。教训：**第三方项目的 PR ≠ 域名验证**，
  域名真伪必须自己实测（三步法写在 `LanzouDomainConfig` 文件头）。
...
- 上传限制：爱企查/php中文网多来源确认免费用户 100MB
- 分卷命名 `.zip/.z01/.z02`（`.001/.002` 被拦截）—— ⚠️ **更正（2026-09-24）**：
  此条依据是 **2020 年第三方博客，并非本人一手实测**（原文写成"博客园实测"易被误读为
  自己验过）。本条已列入 §39.5 的**待真机验证**清单，改动分卷命名逻辑前请先实测。
...
1. ✅ 完整阅读本文件 + `README.md`；**先看「交付前必做 Checklist」与 §39（最近一轮改动）**
2. ✅ 浏览 `docs/README.md`：原始证据文件索引（原版 APK 逆向报告 / 设置项默认值审计 /
   断言可靠性审计 / 功能对照表）。**本文档引用的 `audit_*.md` 等都在仓库 `docs/` 里**
3. ✅ 拉取代码（https://github.com/d1667018881/cloudbox，私有，需 token）
4. ✅ 对照 §9 的参考资料核对 §5 的 task 表是否仍然有效（蓝奏云接口会变）
5. ✅ 核实域名池（§7.4）是否仍可用（被墙/抢注域名及时清理；**验证三步法见 §39.1**）
6. ⚠️ 改动前先看 §6 的坑列表；**改完直接 push 到 main**，CI 自动构建验证（Actions 页面看 run 结果，Release 下载 APK）
7. ⚠️ 发布流程：push main 自动构建发 Release；**不要动 keystore**（动了就无法覆盖安装）
8. ⚠️ **文档里读到互相矛盾的结论时，以章节号大者为准**（本项目按时间线性追加，
   后续章节可能推翻前面的）。若发现旧章节结论已过期，**请顺手加更正注**，不要删原文。
...
## §38 上传文件名加版本号（2026-09-20）

> 🚫 **本节部分结论已于 2026-09-24 作废 —— 动手前必读 §39.1。**
>
> | 本节原文（❌ 已作废） | 现行行为（✅ 以 §39 为准） |
> |---|---|
> | 38.1 表格：`Foo.png` → `Foo-v0.1.157.png`，**所有文件**都插版本号 | **只给 `.apk` 插**（`isSoftwarePackage()`），其它文件原名不动 |
> | 38.2/38.3：伪装 `.zip` **默认开启**，先 `.zip` 再插版本号 | 伪装**默认关闭**（`suffix_spoof_enabled = false`，**与原版一致**）；关闭时不会产生 `.zip`，顺序问题自然消失 |
> | 38.2："蓝奏云只按最后一个扩展名判断能否上传" | **证据不足**：唯一实测样本是「**无扩展名**被拒」，而 **apk 可直接上传**（用户实测）。不能推广成"这些格式都被拒" |
>
> **本节仍然有效的部分**：38.5 `insertVersionBeforeExtension` 的边界规则（尤其 `dot <= 0`
> 同时覆盖无扩展名与前导点）、**"结果的最后一个扩展名恒等于原串最后扩展名"**这条不变量、
> 38.6 的 CI 教训（lambda 返回 `Job` 导致类型不匹配）。这些是实现细节，与默认值无关。
>
> 原文保留不删，是为了留下"当时为什么会这么想"的完整记录 —— 但**不要照抄它的默认值**。

用户需求原话：**「文件名里面，加上上传软件的版本号」**。

一句话需求，实现上却有两个"必须弄对、弄反了会静默出问题"的点。
...
版本号取 `BuildConfig.VERSION_NAME`（CI 注入 `0.1.<run_number>`）。
**不用 `PackageManager`**：那需要 Context、还可能抛异常；BuildConfig 是编译期
常量，取不到就是编译不过，不给运行时留隐患。

### 38.2 坑一：插在「最后一个点之前」，不是追加也不是前缀

三种写法都"看起来对"：

| 写法 | 结果 | 问题 |
|---|---|---|
| 追加 | `Foo.apk-v0.1.157` | ❌ **扩展名不再是最后的后缀** |
| 前缀 | `v0.1.157-Foo.apk` | ⚠️ 破坏按名排序、难认出原文件名 |
| **插在点前** | `Foo-v0.1.157.apk` | ✅ |

追加为什么是错的：蓝奏云**只按最后一个扩展名判断能否上传**（实测无扩展名回
`{"zt":0,"info":"不能上传.格式的文件"}`）。`Foo.apk-v0.1.157` 的最后扩展名是
`157` —— 等于亲手制造一个会被服务端拒掉的文件。而且这个错误**在本地看不出来**，
只有上传时才失败。

### 38.3 坑二：顺序必须是「先版本号、后 `.zip`」

伪装文件的规则是 `原名 + ".zip"`，而下载端还原依赖
**"去掉 `.zip` 就是原名"** 这条等价关系。两个操作的顺序一旦反过来
（先 `.zip` 再插版本号），还原出来的名字就和上传前不一致了。

所以 `applyUploadName` 内部是：先 `needsSpoof` 判定并追加 `.zip`，
**再**统一插版本号。这个顺序是硬约束，不是风格选择。

### 38.4 原始名自带版本号时的取舍

`云匣-1.2.3.apk` → `云匣-1.2.3-v0.1.157.apk.zip`，**原样保留、不去重**。

理由：`1.2.3` 是"被传文件的版本"，`0.1.157` 是"上传工具的版本"，
两者都能提供信息，合并掉（比如检测到已有版本号就跳过）反而丢事实。
多一个 `-v` 段的代价远小于"我上传时用的哪个版本"变得不可考。

### 38.5 边界情况（离线跑了 9 组）

`insertVersionBeforeExtension` 抽成纯函数，就是为了能单独验证它 ——
边界比看上去多：

| 输入 | 结果 | 说明 |
|---|---|---|
| `Foo.png` | `Foo-v0.1.157.png` | 常规 |
| `Makefile` | `Makefile-v0.1.157` | `dot == -1`，无扩展名 |
| `.gitignore` | `.gitignore-v0.1.157` | `dot == 0`，前导点是**隐藏文件标记**而非扩展名 |
| `foo.` | `foo.-v0.1.157` | 以点结尾，`ext` 为空，不插（否则得到 `foo-v1..`） |
| `a.b.c.tar.gz` | `a.b.c.tar-v0.1.157.gz` | 多点，插在真正的最后扩展名前 |
| 空版本号 | 原样返回 | 防御 |

`dot <= 0` 这一个判断同时覆盖 `-1` 和 `0` 两种情况 —— 如果只写
`dot == -1`，`.gitignore` 会变成 `.gitignore` 被拆成 `stem=""`，
产出 `-v0.1.157.gitignore` 这种以连字符开头的怪名字。

**核心不变量：结果的最后一个扩展名恒等于原串的最后扩展名。**
这条不变量才是"上传不会被服务端按扩展名拒掉"的保证，也是我验证时真正在盯的东西。

### 38.6 一次 CI 编译失败，与它暴露的方法问题

推上去的第一次编译 **FAILED**：

```
MainScreen.kt:219:32 Argument type mismatch:
  actual type is 'kotlin.Function0<kotlinx.coroutines.Job>',
  but 'kotlin.Function0<kotlin.Unit>' was expected.
```

源自 §37 的改动：

```kotlin
val doLogout = {              // ← 被推断成 () -> Job
    scope.launch { ... }      //    lambda 体最后一句是 launch，返回值是 Job
}
```

传给 `TextButton` 的 `() -> Unit` 形参即类型不匹配。修法是显式标注
`val doLogout: () -> Unit`（并在末尾补一句 `Unit`）：
**显式类型 + 显式返回值，两处都写，不依赖推断。**

值得记的是**这次失败的价值**：本机没有 Android SDK、没有 `gradlew`，
CI 是唯一的编译器，而 CI 一轮 3~4 分钟。在这种约束下，
最容易漏的就是这类"IDE 会立刻飘红、而这里没人告诉我"的推断型错误。
所以：**凡是把 `scope.launch` 包成 lambda 传给回调的地方，一律显式标注返回类型。**
（已全仓扫过，只有这一处该形状。）

另外顺手记一个 CI 使用的坑：`concurrency` 是串行队列，
**`ci-logs` 分支上的最新日志未必属于你刚推的提交**。
我第一次查结果时读到的是上一个提交（`5ba0eba`）的日志，
差点误判"修了还是失败"。正确做法是拿日志里的提交号跟自己的 HEAD 对一下：

```
git log origin/ci-logs --oneline -1
# 例：9acf07e ci log run 164 (2a1377aca34fef43556b7956dbe3409f155c4fc5)
#                                    ^^^^^^^ 这个才是关键
```
...
| `precheck.py .` | ✅ `OK 检查了 105 个文件，无问题` |
| 括号配平 | ✅ `{}=34/34 ()=128/128` |
| 命名规则边界用例 | ✅ 9 组离线跑通，扩展名不变量成立 |
| CI 编译 | ✅ `BUILD SUCCESSFUL in 3m 4s`（run 164） |
| 提交 | `a0c1318`（功能） + `2a1377a`（编译修复） → `origin/main` |

---

## 39. 「移植原版功能」五批（2026-09-23 ~ 09-24，v0.1.166 → v0.1.179）

背景：把**原版蓝云 App（AndroLua）的功能**移植进 CloudBox，原则是「以原版源码为主，
CloudBox 只算框架」。原版 55 个 `.lua` 已全部解密还原为明文
（工作区 `apk_analysis/src_readable/`），这批改动全部以它为准。

### 39.1 ⚠️ 必须记住的教训：别把「别人的说法」当事实

本轮我自己犯了一个典型错误，与 §20.1 同性质，单列如下：

- **现象**：上传 APK 会被改名成 `xxx-v0.1.x.apk.zip`，用户反馈
  "apk 明明能直接上传，你这么改反而更麻烦"。
- **错在哪（两层）**：
  1. 代码里断言"蓝奏云不接受 exe/apk 等格式"，但**全项目唯一的实测记录**是
     「**无扩展名**的文件被拒」——把一个窄样本推广成了"这些格式都被拒"。
  2. 更糟的是这个功能被设成**默认开启**，注释还写着"对齐原版"。而原版
     `update_log.lua` 1.2.4.0 白纸黑字写着「……**且此功能默认关闭**」。
     原版就是默认关，我"对齐"对反了。
- **改法**：默认改回关闭（v0.1.174）；注释删掉无证据的断言，改写"证据边界"。
- **结论（定默认值前必做）**：先找一手证据（**原版明文 Lua > 本人实测 > 推测**），
  找不到就选**最小干预**（不改名 / 不删除 / 不自动开启）；引用"实测"必须写明
  被验证对象与样本数；声称"对齐原版"必须能给出原版文件:行。

同一轮排查出的同类问题（均已修）：

- **应用内语言是假功能**：`values/strings.xml` 只有 `app_name` 一条，界面文案全部
  硬编码在 Kotlin 里 → 选 English 界面照样是中文。已整个删除，语言跟随系统
  （v0.1.178）。**以后不要再加语言选项，除非先把文案抽成资源。**
- **fallback 域名池混进 3 个非官方域**（`lanzouo / lanzouh / lanzouu`，来源是第三方
  GitHub 项目的 PR，从未验证）→ 实测：`lanzouh/lanzouu` **DNS 根本不存在**，
  `lanzouo` 443 直接 `ConnectionRefused` 且首页无蓝奏云内容 → 全部移除（v0.1.177）。
  保留的 4 个（`lanzou / lanzoui / lanzoup / lanzoux`）都有实测证据；
  **验证三步法**写在 `LanzouDomainConfig` 文件头。
- **文件夹操作菜单根本无法触达**：`ListItem` 的「⋯」按钮被 `!file.isFolder` 排除，
  于是"设置提取码 / 修改资料 / 查看分享链接 / 添加到星标文件夹"全是死代码；
  网格模式同理（v0.1.172 / v0.1.179 分别修复）。
- **3 处引用 `ty_core.lua` 的行号写错**（写 809 / 754 / 709，实际是 1095 / 1040 / 995）
  → 已按 `sed` 实测结果更正（v0.1.179）。

### 39.2 各批内容

| 批次 | 内容 | Release |
|---|---|---|
| 1 | 7 项 P0（下载 Cookie 贯通 / 重命名丢描述 / 搜索可点 / Cookie 导入校验 / 新建文件夹用服务端 id / 二维码含提取码 / 回收站二次确认） | v0.1.166 |
| 2 | 账号面板（task=7/8/10/15）· 应用自更新闭环（改连本仓库 Releases：原版连的 AppCenter 2025-03 停用）· 公告系统（原版外链兔小巢已停运，改仓库内 JSON） | v0.1.167 |
| 3 | 查看全盘文件（task=19 → 逐目录 task=5）· 侧栏抽屉 · 关于页 FAQ/许可 · 图标包(zip)导入 · 4 项列表设置 · 备份增强（备份码 + 增量恢复） | v0.1.168-170 |
| 4 | 下载端 `.zip` 还原 · 冷启动免二次登录 · **星标文件夹**（新表 + DB 6→7 显式迁移） | v0.1.171-173 |
| 5 | 下载保存位置 · 通知栏提醒 · 管理应用通知 · 域名池清理 · 移除语言假功能 · 网格菜单 · 行号修正 | v0.1.174-179 |

### 39.3 新增/变更的设置项（均在 `SettingsStore`）

| 键 | 默认 | 说明 |
|---|---|---|
| `suffix_spoof_enabled` | **false**（本轮由 true 改为 false） | 上传后缀伪装；**原版即默认关** |
| `download_folder` | `""` | 保存到 `Download/<子目录名>`；只允许 Download 下的子目录（分区存储限制） |
| `send_message` | `true` | 通知栏提醒（对齐原版 `send_message`） |

⚠️ 新增设置项**必须同步加进 `snapshotAll` / `applySnapshot`**（备份契约），
否则会出现"恢复备份后该项设置静默丢失"。

### 39.4 数据库版本

`AppDatabase` 现为 **v7**（v7 新增 `starred_folders`，配 `MIGRATION_6_7`）。
**新增表必须写显式迁移**：`fallbackToDestructiveMigration` 在找不到匹配迁移时会
**整库重建**，会连带清掉用户的收藏夹与下载记录。

### 39.5 尚未做 / 需真机确认

- 审计列出的 15 条「断言证据不足」中仍有 6 条**需登录或上传才能验**（分卷命名依据
  2020 年第三方博客、桌面 UA 是否必需、`403=Cookie过期`、每页固定 18 条、
  回收站「2xx 即成功」、「服务端无文件夹移动接口」）。清单在**仓库内**
  [`docs/audit_claims.md`](docs/audit_claims.md) /
  [`docs/audit_settings_defaults.md`](docs/audit_settings_defaults.md)。
- 原版「第三方下载器 / 外部下载管理器 / `api_*` 全套字段 / 铃声提醒」**有意不做**，
  理由见对应 commit message（v0.1.174 / v0.1.176）。
- 本轮所有改动**只过了程序级自检（precheck）+ CI 编译**，**未做实机验证**。

### 39.6 本轮引用的证据文件（已入库 `docs/`）

> 原先这些文件**只存在于开发者沙箱工作区**，而 §39 正文却直接引用它们 ——
> 新接手的 AI 打开仓库根本找不到，等于引用了一堆不存在的东西。2026-09-25 全部入库。

| 文件 | 用途 |
|---|---|
| [`docs/original_apk_reverse_report.md`](docs/original_apk_reverse_report.md) | 原版 APK 逆向报告：**55 个明文 Lua 的来源与还原方法**（两层加密、90,066 行）+ 逐文件功能分析。本轮"以原版为准"的**唯一一手依据** |
| [`docs/original_settings_inventory.md`](docs/original_settings_inventory.md) | 原版「下载设置」12 项 + 「消息设置」10 项逐项清单（带原版文件:行出处） |
| [`docs/audit_settings_defaults.md`](docs/audit_settings_defaults.md) | 全设置项**默认值 vs 原版**对照审计（一致 13 / 不一致 2 / 无法核对 3） |
| [`docs/audit_claims.md`](docs/audit_claims.md) | 全仓「实测 / 对齐原版」类断言可靠性审计（约 201 处命中，15 条存疑） |
| [`docs/feature_parity_table.md`](docs/feature_parity_table.md) | 云匣 ↔ 蓝云 全功能对照表（105 个 Kotlin 文件 ↔ 原版 Lua） |

索引与使用说明见 [`docs/README.md`](docs/README.md)。

### 39.7 文档自查发现并修正的过期结论（2026-09-25）

交接文档写完后自查了一遍，发现**旧章节仍留着已被推翻的结论**——新 AI 按顺序读会被误导：

| 位置 | 问题 | 处置 |
|---|---|---|
| §38 | 38.1/38.2 写"所有文件都加版本号 + 伪装默认开"，已被本轮推翻，且原文无任何提示 | 节首加**作废对照表** |
| §9.1 | 拿「PR#69 修复 `lanzouo.com`」佐证"接口有效"，而该域实测是**假域** | 加更正注 |
| §9.4 | 「分卷命名…博客园实测」，实为 2020 年**第三方博客**，非本人实测 | 加更正注 |

**由此确立的文档规矩**：

1. 本文件按时间**线性追加**，**后出现的章节可能推翻前面的**；读到矛盾**以章节号大者为准**。
2. 发现旧结论过期 → **加更正注，不删原文**（决策链本身是资产，删了就看不出"当时为什么这么想"）。
3. 引用仓库外的文件前，**先确认它真的在仓库里**——否则接手者拿不到。

---

## 40. V40 长间隔全量复审（2026-09-25，复审 AI 交接，v0.1.182 基线）

背景：9/3 v0.1.99 之后由接手 AI 连续修复/移植（89 提交，+18604/-854，v0.1.182）。
本轮为换手后的全量复审：已知问题销账对账 + 新增量分层读 + 能修即修。

### 40.1 已知问题销账（全部核实已修，无漏项）

| 来源 | 问题 | 现场 |
|---|---|---|
| §36.2 #1 | 会话恢复中断文件数被清零（假成功） | 已修：阶段化记账 + active.isEmpty 立即裁决 |
| §36.2 #2 | globalTotal/initialFinished 口径不一致 | 已修：均按非 CANCELLED |
| §36.2 #3 | WebView 上传返回键 resultCode 不一致 | 已修：onBackPressed 统一 finishOk |
| §37 | 改密码后被静默重登复活 | 已修：Failure→markRejected，removeUid 联动清槽位 |
| §35.1-35.3 | 清缓存删在途源文件 / 半完成状态 / 裸调异常 | 已修：hasUploadInFlight 守卫 + 动手前检查 + runCatching |
| §39.1 | 后缀伪装默认值错 / 假域 deep link | 已修：默认关 + Manifest 三假域移除（与域名池同步） |

### 40.2 本轮新发现并当场修复

| # | 级别 | 位置 | 问题 | 修复 |
|---|---|---|---|---|
| N1 | 中 | SettingsStore.snapshotAll/applySnapshot | **五批移植新增的 5 个用户可见设置项漏进备份快照**（图标包路径、描述标签、双行标题、剪贴板识别、删除二次确认）——恢复备份后这些设置静默回默认值。正是 snapshotAll 注释警告过的坑，§39.3 立过规矩但新代码没跟上 | 两处各补 5 行；iconPackPath 恢复时无效路径由 FileIconResolver 自动回落默认图标，无崩溃面 |
| N2 | 低 | FileListViewModel.loadPage | append 失败不回退 page → 下次 loadMore 跳过一整页文件（静默丢数据） | 失败且 seq 匹配时 page-- |
| N3 | 低 | IconPackStore.importZip | 只限条目数（2000）不限**解压总量**——zip bomb 可把外部存储写满 | 逐块累计实写上限 50MB |
| N4 | 注 | DirectLinkWebViewBridge | 头注释声称"同一时刻只允许一个挑战（synchronized）"——实现里从未有锁，注释与代码不符 | 更正为"并发不设锁（各自独立、不损坏正确性）" |

### 40.3 核实无误的高危面（重点确认没被改坏）

- 备份加密（BackupCrypto）：PBKDF2-SHA256 10 万次 + 随机 salt + 随机 IV + AES-GCM，密码不落盘 ✓
- 备份写入：临时文件 + 回读校验 + 同文件系统原子改名 ✓
- ZipSlip：canonical + startsWith 前缀校验 + MAX_ENTRIES ✓（N3 补上总量）
- DB：v4→v7 三条显式迁移，收藏/下载记录/星标不落 destructive 兜底 ✓
- 上传判定：zt==1 且须返回 fileId + verifyOnCloud 云端验证兜底；上传名版本号插入的 5 类边界（无扩展名/前导点/尾点/多段点/空版本）全处理 ✓
- 拦截器链序：retry→UA→Referer→uid→domain（重写最内层）✓；hasMore 判据=页满 18 条（服务端分页机制直接推论）✓
- 全盘加载：串行 + 300ms/目录节流 + hasMore 修正后无死循环面 ✓
- 退出登录：清 Cookie+槽位+星标（按该账号自己的 uid，不依赖 currentUid）✓

### 40.4 不修项（带理由）

| # | 项 | 理由 |
|---|---|---|
| N5 | ErrorActivity 用 Compose，理论上有"错误页自己崩"的二次面 | CLEAR_TASK + 无自动重启，循环链实际断开；该页无 DI、无复杂初始化，Compose setContent 稳定性足够；换纯 View 重写的回归风险 > 收益 |
| N6 | 图标包副本（uploaded_apps/）上传成功后不清，等下次打开选择器才 purge | cacheDir 语义（系统可回收）+ 立删会与在途拷贝竞态（§V32 已论证），现状是有意设计 |

### 40.5 交付

修复含本轮 4 项（N1-N4），`precheck.py` 137 文件全绿，CI 出包即验。
真机验收补充项：改 5 项新设置（图标包/描述标签/双行标题/剪贴板/删除确认）→ 导出备份 → 恢复 → 确认设置保留。

---

## 41. 发行资产与说明自动化（2026-09-26，用户要求）

build.yml 的 Create Release 步骤改为：

1. **资产名 = `CloudBox-<tag>.apk`**（如 `CloudBox-v0.1.184.apk`），不再是 gradle
   默认的 `app-release.apk`。软件名与版本号都从 TAG 取（TAG 是唯一版本来源），
   不会出现文件名与发行版对不上。
2. **发行说明 = 本次构建对应的 commit message**（首行简述 + 正文细节）。

**为什么用 commit message 当发行说明（通用性设计）**：commit message 是任何
AI 工具/人提交代码时都会写的东西 —— 「每次发版写清楚说明」这个责任自动落在
提交信息上，换接手者无需记忆任何额外约定，也不会再出现 "Auto build APK"
这类零信息文案。**推论：提交信息就是发行说明，写得越规范发行版越可读。**

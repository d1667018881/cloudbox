 ...
 *
 * 审查修复（CODE_REVIEW #18）：主键改为 (accountUid, id) 复合主键——
 * 旧实现用自增 rowId 做主键，OnConflictStrategy.REPLACE 永不触发（每次都是新行），
 * 翻页时目录列表重复堆积、缓存表无限膨胀。
 *
...
@Entity(
    ...
    primaryKeys = ["accountUid", "id"],
    indices = [Index("parentId"), Index("accountUid"), Index("name")]
)

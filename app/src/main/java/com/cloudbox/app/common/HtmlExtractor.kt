...
object HtmlExtractor {
     ...
     * 新版 k：随机变量名 = 长 hex 串。
     *
     * 证据（两个样本，2026-09-24 复核）：
     * - `var _hjuio = '368d7ce8…'`（首次反编译所得）
     * - `var _hk7mw = 'a30020e553f05fcbecbd7e51c10302ce';`（2026-09-24 抓
     *   https://wwe.lanzoui.com/b01tpeg7i 实测，**32 位纯 hex**，本正则命中）
     *
     * 变量名允许下划线、长度不限 —— 旧版 `{6}` 定长且不接受下划线，正是
     * 匹配失败的原因。
     *
     * ⚠️ 已知风险：此处仍假定 k 是**纯 hex**。若将来分享页改用含 g-z 的随机串，
     * 本正则与 RE_K_OLD 会一起失配，表现为"解析失败"。真出现解析失败时，
     * 这里是第一个该检查的地方。
     */
